...
from detail.format_details import get_currency, format_date, get_symbol_currency, find_directly_start_date1, find_date, \
    filter_locations
from download_parse import download_site


async def get_version_info(course,course_obj,session):
    '''
    :param course_obj
    :return: practical info page link
    '''
    no_version_url = ["https://www.vlerick.com/en/programmes/management-programmes/human-resource-management/vlerick-hr-day",
    ...
    directly_version_url = ["https://www.vlerick.com/en/programmes/management-programmes/accounting-finance/excellence-in-corporate-finance",
    ...
    elif course["url"] in directly_version_url:
        version_info = await directly_version_info(course,session)
        return version_info
    version_info = {}
    ...
    if practical_a_tag:
        ...
        practical_page = await download_site(practical_info_link,session)
...
def location_for_one_version_course(practical_info_page_obj):
    ...
    try:
        ...
        print(f'location has some problem of: {e}')
    return [location]
...
def course_version_info_way1(course,course_obj, practical_info_page_obj):
    ...
    try:
        ...
        print(f'{course["url"]} first find_all has problems {e}')
    if len(locations) != 0:
...
async def directly_version_info(course,session):
    version_list = []
    version_page_url = course['url'] + '/practical-info'
    page = await download_site(version_page_url,session)
...
def get_directly_version1(page):
    ...
    date_session = date_session.next_sibling.find('li')
