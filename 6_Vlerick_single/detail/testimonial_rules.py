...
async def get_testimonial_info(course,course_obj,session):
    testimonial_list = []
    try:
        if course['url'] == "https://www.vlerick.com/en/programmes/management-programmes/digital-transformation/digital-leadership":
            ...
            testimonial_page = await get_testi_page_url(course_obj,session)
...
async def get_testi_page_url(course_obj,session):
    url = ''
...
# testi_url = "https://www.vlerick.com/en/programmes/management-programmes/people-management-leadership/negotiate-for-success"
# source = requests.get(testi_url).content
# page = bs4.BeautifulSoup(source,'lxml')
# testis = get_negotiate_for_success_testis(testi_url,page)
